...
(* pattern matching. 
 * use the random value created by rand()
 * when depth is 0, buildX() / buildY()
*)
let rec build (rand,depth) = 
  match rand(1,6) with
  | 1 when depth = 0 -> buildX() 
  | 1 -> buildSine(build(rand,depth -1))
  | 2 when depth = 0 -> buildY()
  | 2 -> buildCosine(build(rand,depth -1))
  | 3 when depth = 0 -> buildX()
  | 3 -> buildAverage(build(rand,depth -1), build(rand,depth -1))
  | 4 when depth = 0 -> buildY()
  | 4 -> buildTimes(build(rand,depth -1), build(rand,depth -1))
  | 5 when depth = 0 -> buildX()
  | 5 -> buildThresh(build(rand,depth -1),build(rand,depth -1),build(rand,depth -1),build(rand,depth -1))

(* pattern matching. 
 * new version with more operators
*)
let rec build2 (rand,depth) = 
  ...
  | 5 when depth = 0 -> buildX()
  | 5 -> buildThresh(build(rand,depth -1),build(rand,depth -1),build(rand,depth -1),build(rand,depth -1))
  | 6 when depth = 0 -> buildY()
  | 6 -> buildCube(build(rand,depth -1))
  | 7 when depth = 0 -> buildX()
  | 7 -> buildRandom(build(rand,depth -1), build(rand,depth -1), build(rand,depth -1))
